<?php
    ...
    $ID_MARCA = $_GET['id'];
    $consultaSQL = "SELECT * FROM marca WHERE ID_MARCA =" . $ID_MARCA;
    ...
    if (!$marca) {
        $resultado['error'] = true;
        $resultado['mensaje'] = 'No se ha encontrado el marca ❌';
    }
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                ...
                El marca ha sido actualizado correctamente, Verifica los cambios en el siguiente enlace: 
                    <a class="btn btn-success" href="../../adm/marcas.php">Regresar al inicio</a>
                </div>
            </div>
        </div>
    </div>
...
?>
  ...
  <div class="container-fluid formulario_edicion">
    <div class="row ">
      <div class="col-md-12 ">
      <h2 class="text-center">Editando Datos de <?= escapar($marca['NOMBRE_MARCA']) ?>
    <?php if ($marca['ID_MARCA'] != 1 && $marcas[$marca['ID_MARCA']] != '-SIN MARCA') { ?>
        <?= ' ' . escapar($marcas[$marca['ID_MARCA']]) ?>
    <?php } ?>
</h2>
        ...
        <form method="post" enctype="multipart/form-data">
            <div class="row">
                <div class="col-12 col-sm-6 col-md-4 col-lg-4">
                    <div class="form-group">
                        <label for="nombre">No.Marca</label>
                        <input type="text" name="id_producto" ID_MARCA="id_producto" value="<?= escapar($marca['ID_MARCA']) ?>" class="form-control" readonly>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-md-4 col-lg-4">
                    <div class="form-group">
                        ...
                        <option value="" disabled selected>Selecciona una opción</option>
                            <?php foreach ($categorias as $id => $nombre) { ?>
                                <option value="<?= $id ?>" <?= $id == $marca['ID_CATEGORIA'] ? 'selected' : '' ?>>
                                    <?= escapar($nombre) ?>
                                </option>
                            <?php } ?>
                        </select>
                ...
                <div class="col-12 col-sm-6 col-md-4 col-lg-4">
                    <div class="form-group">
                        ...
                        <input type="text" name="nombre" ID_MARCA="nombre" value="<?= escapar($marca['NOMBRE_MARCA']) ?>" class="form-control" required>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="marca" name="marca" class="form-select" aria-label="Default select example" required>
                            ...
                            <?php foreach ($marcas as $id => $nombre) { ?>
                                <option value="<?= $id ?>" <?= $id == $marca['ID_MARCA'] ? 'selected' : '' ?>>
                                    <?= escapar($nombre) ?>
                                </option>
                            <?php } ?>
                        </select>
                ...
                <div class="col-12 col-sm-6 col-md-4 col-lg-4">
                    <div class="form-group">
                        ...
                        <input type="text" name="presentacion" ID_MARCA="presentacion" value="<?= escapar($marca['PRESENTACION']) ?>" class="form-control" required>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-md-4 col-lg-4">
                    <div class="form-group">
                        <label for="precio">Precio</label>
                        <input type="text" name="precio" ID_MARCA="precio" value="<?= escapar($marca['PRECIO']) ?>" class="form-control" required>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-md-4 col-lg-4">
                    <div class="form-group">
                        ...
                        <select id="estado" name="estado" class="form-select" aria-label="Default select example"  required>
                            <option value="" disabled selected>Selecciona una opción</option>
                            <option value="Disponible" <?php if ($marca['ESTADO'] === 'Disponible') echo 'selected'; ?>>Disponible</option>
                            <option value="No Disponible" <?php if ($marca['ESTADO'] === 'No Disponible') echo 'selected'; ?>>No Disponible</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-12 col-sm-6 col-md-4 col-lg-4">
                    <div class="form-group">
                        <label for="foto_producto">Foto Marca</label>
                        <div class="input-group mb-3">
                            <input type="file" class="form-control" id="inputGroupFile02" name="foto_producto" ID_MARCA="foto_producto" value="<?= escapar($marca['FOTO_PRODUCTO']) ?>" accept='image/*' onchange="previewImage(event)">
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12 col-sm-12 col-md-12 col-lg-12">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="descripcion" id="descripcion" class="form-control" required><?= escapar($marca['DESCRIPCION']) ?></textarea>
                    </div>
                </div>
          ...
          <div class="form-group my-3">
            ...
            <a class="btn btn-primary" href="../../adm/marcas.php">Regresar al inicio</a>
          </div>
        </form>
      </div>
    </div>
...
if(isset($_FILES['foto_producto']) && $_FILES['foto_producto']['error'] == 0){
    ...
    if(move_uploaded_file($_FILES['foto_producto']['tmp_name'], $nueva_ruta)){
        ...
        $stmt = $conn->prepare("UPDATE marca SET FOTO_PRODUCTO = ? WHERE ID_MARCA = ?");
...
?>
  <?php
        $id_producto = $_GET['id'];
            // Consulta para obtener la extensión de la imagen del marca
            $stmt = $conn->prepare("SELECT FOTO_PRODUCTO FROM marca WHERE ID_MARCA = ?");
            $stmt->execute([$id_producto]);
            $marca = $stmt->fetch();

            // Construye la ruta de la imagen
            $ruta_imagen = "imagenes/" . $id_producto . "." . $marca['FOTO_PRODUCTO'];
           
            ?>
            <img src="<?= $ruta_imagen ?>" alt="Foto del marca" class="foto_actual">
             <style>
                .foto_actual {
                    position: absolute;
                    top: 0;
