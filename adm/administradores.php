<?php
  ...
  try {
    ...
    if (isset($_POST['NUMERO_DOCUMENTO']) && !empty($_POST['NUMERO_DOCUMENTO'])) {
      $consultaSQL = "SELECT * FROM administrador WHERE NUMERO_DOCUMENTO LIKE '%" . $_POST['NUMERO_DOCUMENTO'] . "%'";
  } else {
      $consultaSQL = "SELECT * FROM administrador";
  }
    ...
    $administrador = $sentencia->fetchAll();

  } catch(PDOException $error) {
    $error= $error->getMessage();
  }

  $titulo = isset($_POST['NUMERO_DOCUMENTO']) ? 'Documento Buscado(' . $_POST['NUMERO_DOCUMENTO'] . ')' : 'Listado de Administradores';
?>
<!DOCTYPE html>
<html lang="en">
...
<main class="container-fluid">
  <div class="bg-light rounded">
    <div class="acciones">
      <a href="../controllers/administradores/crear.php"  class="btn btn-primary mt-4">Nuevo Administrador</a>
              ...
              <tbody>
                ...
                if ($sentencia && $sentencia->rowCount() > 0) {
                  foreach ($administrador as $fila) {
                    ?>
                    <tr>
                      <td><?php echo escapar($fila["ID_ADMINISTRADOR"]); ?></td>
                      <td><?php echo escapar($fila["NOMBRE_ADMINISTRADOR"]); ?></td>
                      <td><?php echo escapar($fila["APELLIDO_ADMINISTRADOR"]); ?></td>
                      ...
                      <td class="acciones">
                        <a href="<?= '../controllers/administradores/eliminar.php?id=' . escapar($fila["ID_ADMINISTRADOR"]) ?>">🗑️Borrar</a>
                        <a href="<?= '../controllers/administradores/editar.php?id=' . escapar($fila["ID_ADMINISTRADOR"]) ?>">✏️Editar</a>
                      </td>
                    </tr>
                    <?php
                  }
...
<div class="container-fluid resultados-tarjetas" id="formato-tarjetas" style="display: none;">
  ...
  if ($sentencia && $sentencia->rowCount() > 0) {
      ...
      foreach ($administrador as $fila) {
          ...
          $ruta_imagen = "../controllers/administradores/perfiles/" . $id_administrador. "." . $administrador['FOTO_ADMINISTRADOR'];
          ?>
          <div class="col-lg-3 col-md-6 mb-4">
              <div class="card h-100">
                  <img src="<?= $ruta_imagen ?>" class="card-img-top" alt="Foto del administrador <?php echo escapar($fila["ID_ADMINISTRADOR"]); ?> " style="margin: auto; width: 200px; height: 200px; border-radius: 100px;">
                  <div class="card-body">
                      <h5 class="card-title"><?php echo escapar($fila["NOMBRE_ADMINISTRADOR"]) . " " . escapar($fila["APELLIDO_ADMINISTRADOR"]); ?></h5>
                  ...
                  <div class="card-body">
                      <a href="<?= '../controllers/empleados/eliminar.php?id=' . escapar($fila["ID_ADMINISTRADOR"]) ?>" class="card-link">🗑️Borrar</a>
                      <a href="<?= '../controllers/empleados/editar.php?id=' . escapar($fila["ID_ADMINISTRADOR"]) ?>" class="card-link">✏️Editar</a>
                  </div>
              </div>
          </div>
          <?php
