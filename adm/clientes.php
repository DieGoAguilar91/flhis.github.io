<?php
  ...
  try {
    ...
    if (isset($_POST['NUMERO_DOCUMENTO']) && !empty($_POST['NUMERO_DOCUMENTO'])) {
      $consultaSQL = "SELECT * FROM cliente WHERE NUMERO_DOCUMENTO LIKE '%" . $_POST['NUMERO_DOCUMENTO'] . "%'";
  } else {
  ...
  $titulo = isset($_POST['NUMERO_DOCUMENTO']) ? 'Documento Buscado(' . $_POST['NUMERO_DOCUMENTO'] . ')' : 'Listado de Clientes';
?>
<!DOCTYPE html>
<html lang="en">
...
<main class="container-fluid">
  ...
  <div class="bg-light rounded">
    <a href="../controllers/clientes/crear.php"  class="btn btn-primary mt-4">Nuevo Cliente</a>
                ...
                if ($sentencia && $sentencia->rowCount() > 0) {
                  foreach ($cliente as $fila) {
                    ...
                    <tr>
                      ...
                      <td><?php echo escapar($fila["APELLIDO_CLIENTE"]); ?></td>
                      ...
                      <td class="acciones">
                        <a href="<?= '../controllers/clientes/eliminar.php?id=' . escapar($fila["ID_CLIENTE"]) ?>">🗑️Borrar</a>
                        <a href="<?= '../controllers/clientes/editar.php?id=' . escapar($fila["ID_CLIENTE"]) ?>">✏️Editar</a>
                      </td>
                    </tr>
                    <?php
                  }
...
<div class="container-fluid resultados-tarjetas" id="formato-tarjetas" style="display: none;">
  ...
  if ($sentencia && $sentencia->rowCount() > 0) {
      ...
      foreach ($cliente as $fila) {
          ...
          $ruta_imagen = "../controllers/clientes/perfiles/" . $id_cliente . "." . $cliente['FOTO_CLIENTE'];
          ?>
          <div class="col-lg-3 col-md-6 mb-4">
              <div class="card h-100">
                  <img src="<?= $ruta_imagen ?>" class="card-img-top" alt="Foto del cliente <?php echo escapar($fila["ID_CLIENTE"]); ?> " style="margin: auto; width: 200px; height: 200px; border-radius: 100px;">
                  <div class="card-body">
                      <h5 class="card-title"><?php echo escapar($fila["NOMBRE_CLIENTE"]) . " " . escapar($fila["APELLIDO_CLIENTE"]); ?></h5>
                  ...
                  <div class="card-body">
                      <a href="<?= '../controllers/clientes/eliminar.php?id=' . escapar($fila["ID_CLIENTE"]) ?>" class="card-link">🗑️Borrar</a>
                      <a href="<?= '../controllers/clientes/editar.php?id=' . escapar($fila["ID_CLIENTE"]) ?>" class="card-link">✏️Editar</a>
                  </div>
              </div>
          </div>
          <?php
