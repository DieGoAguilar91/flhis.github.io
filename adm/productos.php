<?php
  ...
  try {
    ...
    if (isset($_POST['NOMBRE_PRODUCTO']) && !empty($_POST['NOMBRE_PRODUCTO'])) {
      $consultaSQL = "SELECT * FROM producto WHERE NOMBRE_PRODUCTO LIKE '%" . $_POST['NOMBRE_PRODUCTO'] . "%'";
  } else {
      $consultaSQL = "SELECT * FROM producto ORDER BY NOMBRE_PRODUCTO ASC" ;
  }
  ...
  $titulo = isset($_POST['NOMBRE_PRODUCTO']) ? 'Resultados de (' . $_POST['NOMBRE_PRODUCTO'] . ')' : 'Listado de Productos';
...
<main class="container-fluid">
  ...
  <div class="bg-light rounded">
    <a href="../controllers/productos/crear.php"  class="btn btn-primary mt-4">Nuevo Producto</a>
          ...
          <div class="row">
            <div class="col-6">
                <div class="form-group mr-3">
                  <input type="text" id="NOMBRE_PRODUCTO" name="NOMBRE_PRODUCTO" placeholder="Buscar por Nombre de Producto" class="form-control">
                </div>
            </div>
            <div class="col-6">
              ...
              if ($sentencia && $sentencia->rowCount() > 0) {
                foreach ($producto as $fila) {
                    ...
                    <tr>
                        ...
                        <td><?php echo escapar($categorias[$fila["ID_CATEGORIA"]]); ?></td>
                        <td><?php echo escapar($fila["NOMBRE_PRODUCTO"]); ?></td>
                        <td><?php echo escapar($marcas[$fila["ID_MARCA"]]); ?></td>
                        <td><?php echo escapar($fila["PRESENTACION"]); ?></td>
                        <td><?php echo "$ " . escapar($fila["PRECIO"]); ?></td>
                        <td><?php echo escapar($fila["ESTADO"]); ?></td>
                        <td class="acciones">
                            <a href="<?= '../controllers/productos/eliminar.php?id=' . escapar($fila["ID_PRODUCTO"]) ?>">🗑️Borrar</a>
                            <a href="<?= '../controllers/productos/editar.php?id=' . escapar($fila["ID_PRODUCTO"]) ?>">✏️Editar</a>
                        </td>
                    </tr>
                    <?php
                }
...
<div class="container-fluid resultados-tarjetas" id="formato-tarjetas" >
  ...
  if ($sentencia && $sentencia->rowCount() > 0) {
      ...
      foreach ($producto as $fila) {
          ...
          $ruta_imagen = "../controllers/productos/imagenes/" . $id_producto . "." . $producto['FOTO_PRODUCTO'];
          ?>
       <div class="col-12 col-sm-6 col-md-4 col-lg-3  mb-4">
    <div class="card h-100">
        <img src="<?= $ruta_imagen ?>" class="card-img-top" alt="Foto del Producto <?php echo escapar($fila["ID_PRODUCTO"]); ?>">
        <div class="card-body">
            <h5 class="card-title"><?php echo escapar($fila["NOMBRE_PRODUCTO"]); ?></h5>
        ...
        <div class="card-body">
            <a href="<?= '../controllers/productos/eliminar.php?id=' . escapar($fila["ID_PRODUCTO"]) ?>" class="card-link">🗑️Borrar</a>
            <a href="<?= '../controllers/productos/editar.php?id=' . escapar($fila["ID_PRODUCTO"]) ?>" class="card-link">✏️Editar</a>
        </div>
    </div>
</div>
